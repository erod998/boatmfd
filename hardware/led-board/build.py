...
(anywhere else, set KICAD_DIR to KiCad's install root first: see kicadpaths.py)
  ...
  1. footprints.py  -> led-board.pretty (the project footprint library)
  2. schematic.py   -> led-board.kicad_sch, then KiCad's ERC, which must be clean
  3. board.py       -> led-board.kicad_pcb (+ .kicad_pro, .kicad_dru), then KiCad's DRC with
...
def through_hole(footprint):
    """The output connectors, the input terminal, the RJ45 and the fuse holders."""
    return footprint.split(":")[0].startswith(("Connector_Phoenix", "TerminalBlock", "Connector_RJ", "Fuse"))
...
def package(footprint):
    ...
    if lib.startswith(("Connector", "TerminalBlock", "Fuse")):
        return name
    chip = re.match(r"[RC]_(\d{4})_", name)
    if chip:
        return chip.group(1)
    if name.startswith("Crystal_SMD"):
        return name.split("_")[2]             # "3225-4Pin"
    if name.startswith("R_Shunt"):
        return "2512 (4-terminal)"
...
MANUFACTURERS = [("CL05", "Samsung Electro-Mechanics"), ("CL21", "Samsung Electro-Mechanics"),
                 ("CL31", "Samsung Electro-Mechanics"), ("CL32", "Samsung Electro-Mechanics"), ("RC", "Yageo"),
                 ("BAT54S", "Nexperia"), ("BUK9M", "Nexperia"), ("PCA96", "NXP"), ("BSS138", "onsemi"),
                 ("SMCJ", "Littelfuse"), ("SS14", "onsemi"), ("SN74", "Texas Instruments"), ("INA226", "Texas Instruments"),
                 ("AP63205", "Diodes Incorporated"), ("AP2112", "Diodes Incorporated"), ("RP2040", "Raspberry Pi"),
                 ("W25Q16", "Winbond"), ("ABM8", "Abracon"), ("KMR2", "C&K"), ("GCT", "GCT"), ("WSK", "Vishay Dale"),
                 ("SRN6045", "Bourns"), ("EEE-", "Panasonic"), ("LTST", "Lite-On"), ("Amphenol", "Amphenol"),
                 ("Littelfuse", "Littelfuse"), ("METZ", "METZ CONNECT"), ("Phoenix Contact", "Phoenix Contact")]
...
def pcbway_files():
    ...
    for (value, fp, mpn), parts in groups.items():
        ...
        tht = through_hole(fp)
        part_no = mpn.split(" (")[0]
        for maker in ("Phoenix Contact ", "Littelfuse ", "Amphenol ", "METZ CONNECT ", "GCT "):
            part_no = part_no.replace(maker, "")
        part_no = part_no.split(";")[0]
        notes = []
        if tht:
            notes.append("through-hole")
        if parts[0].ref == "J11":
            notes.append("surface-mount, with through-hole shell pins")
        if parts[0].ref == "U2":
            notes.append("the flash (U3) is programmed over USB after assembly: no pre-programming")
        if parts[0].ref == "Y1":
            notes.append("12 MHz, 3.2x2.5 mm, 10 pF load (Raspberry Pi's choice for the RP2040)")
        if parts[0].ref == "R47":
            notes.append("four-terminal 1 mOhm shunt: pins 2 and 3 are the sense (Kelvin) terminals")
        if "plug" in mpn:
...
def main():
    ...
    print("design")
    print("  (board: 2 layers, 2 oz copper outside -- the 20 A paths need it; say so on the PCB order)")
